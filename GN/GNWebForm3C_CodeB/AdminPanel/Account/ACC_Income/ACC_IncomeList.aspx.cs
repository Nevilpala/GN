...
public partial class AdminPanel_ACC_Income_ACC_IncomeList : System.Web.UI.Page
{
    ...
    protected void rpData_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        if (e.CommandName == "DeleteRecord")
        {
            try
            {
                DeleteExpenseById(e.CommandArgument.ToString());
            }
            catch (Exception ex)
            {
    ...
    protected void btnDeleteSelected_Click(object sender, EventArgs e)
    {
        ...
        {
            var checkBox = (CheckBox)item.FindControl("chkSelect");
            HiddenField hfId = (HiddenField)item.FindControl("hfID"); // Assuming you have a HiddenField for ID

            if (checkBox != null && checkBox.Checked && hfId != null && !string.IsNullOrEmpty(hfId.Value.ToString()))
            {
                selectedIds.Add(hfId.Value);
            }
        }
        ...
        if (selectedIds.Any())
        {
            ...
            {
                DeleteExpenseById(id.ToString());
            }

            // Optionally, rebind data to refresh the list 
        }
    }

    private void DeleteExpenseById(string id)
    {
        try
        {
            ACC_IncomeBAL balACC_Income = new ACC_IncomeBAL();
            if (id.Trim() != "")
            {
                if (balACC_Income.Delete(Convert.ToInt32(id)))
                {
