...
<asp:Content ID="Content2" ContentPlaceHolderID="cphPageHeader" runat="Server">
    <asp:Label ID="lblPageHeader_XXXXX" Text="Master Dashboard Wise All Finyear" runat="server"></asp:Label>
...
<asp:Content ID="Content3" ContentPlaceHolderID="cphBreadcrumb" runat="Server">
    ...
    <li>
        ...
        <asp:HyperLink ID="hlHome" runat="server" NavigateUrl="~/AdminPanel/Dashboard3.aspx" Text="Dashboard"></asp:HyperLink>
            ...
            <div class="portlet box blue">
                <div class="portlet-title">
                    <div class="caption font-white">
                      <%--  <asp:Label SkinID="lblViewFormHeaderIcon" runat="server"></asp:Label>
                        <asp:Label ID="lblSearchHeader" SkinID="lblSearchHeaderText" Text="FinYear Wise Analysis" runat="server"></asp:Label>
                       --%> 
                        <i class="fa fa-line-chart"></i>
                        <span class="caption-subject bold uppercase">FinYear Wise Analysis</span>
                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                            ...
                                            <div class="portlet box yellow">
                                                <div class="portlet-title">
                                                    <div class="caption">
                                                        <i class="fa fa-h-square "></i><%# Eval("FinYearName") %>
                                                    </div>
                                                    <div class="tools">
                                                                        ...
                                                                        <div class="portlet-body form">
                                                                            <div class="form-horizontal" role="form">
                                                                                <div class="form-body">
                                                                                    ...
                                                                                    <div class="">
                                                                                        <div id='chart_div_<%# Container.ItemIndex %>' style="width: 100%; height: auto;"></div>
                                                                                    </div>
...
<asp:Content ID="Content5" ContentPlaceHolderID="cphScripts" runat="Server">
    ...
    <%--<script type="text/javascript">
        ...
        function drawChart() {
            ...
            var options = {
                ...
                'width': 600,
                'height': 400,
                'bars': 'horizontal' // To make the chart horizontal
            };
    ...
    <%--<script type="text/javascript"> 
        ...
        // Populate chartData from server-side

        var chartDataArray = [];
        // Populate chartData from server-side
            <% 
        // Loop through Repeater items and generate JavaScript array
        foreach (RepeaterItem item in rpFinYear.Items)
        {
            HiddenField hdFinyearID = (HiddenField)item.FindControl("hdFinyearID");
            string finYearID = hdFinyearID.Value;
            string divID = "chart_div_" + item.ItemIndex;
            Response.Write("chartDataArray.push({ id: '" + finYearID + "', divId: '" + divID + "' });");
        }
            %>
        google.charts.load('current', { 'packages': ['corechart'] });

        google.charts.setOnLoadCallback(function () {
            drawChartsForAllHospitals(chartDataArray);
        });
        ...
        function drawChartsForAllHospitals(chartDatas) {
            // Iterate over each Repeater item
            chartDatas.forEach(function (data) {
                var finYearID = data.id;
                var divId = data.divId;

                // Fetch or prepare the data for this FinYearID
                var chartDataForID = window['chartData_' + finYearID];

                // Ensure chartData exists before attempting to draw the chart
                if (chartDataForID) {
                    drawChart(divId, chartDataForID);
                } else {
                    console.log('No data available for FinYearID:', finYearID);
                }
            });
    ...
    <%--        <script type="text/javascript">
            ...
            google.charts.setOnLoadCallback(function () { 
                drawChart('chart_div_0', chartData_0); 
                drawChart('chart_div_1', chartData_0);
                drawChart('chart_div_2', chartData_0); 
            });
            ...
            function drawChart(containerId, chartData) {
                ...
                if (!google.visualization || !google.visualization.DataTable) {
                    console.error("Google Charts library is not loaded correctly.");
                    return;
                }

                if (!chartData || chartData.length === 0) {
                    console.error(`No chart data available for container: ${containerId}`);
                    return;
                }
                ...
                let data = chartData.map(function (row) {
                    return [row.Hospital, row.TotalIncome, row.TotalExpense];
                });
                dataTable.addRows(data);
            ...
            { %>
                console.log('chart_div_<%= i %>', chartData_<%= i %>);
    ...
    <script>
        ...
        function drawChart(containerId, chartData) {
            ...
            var options = {
                ...
                hAxis: {
                    title: 'Hospital', // Leave empty if no title needed for the X-axis
                ...
                chartArea: { width: '90%', height: '70%' }, // Adjust chart area to fit better
                bar: { groupWidth: '50%' }, // Adjust bar width
                annotations: {
                    alwaysOutside: true, // Make sure labels are outside the bars
                    textStyle: {
